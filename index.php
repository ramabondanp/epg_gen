...
foreach($arr_days as $days) {
	...
	foreach($arr_elements as $key => $element) {
		...
		if(strcmp($dates,'day2_schedule')==0){
			$date = date("Ymd", strtotime("+1 day"));
		}else
		if(strcmp($dates,'day3_schedule')==0){
			$date = date("Ymd", strtotime("+2 day"));
		}else
		if(strcmp($dates,'day4_schedule')==0){
			$date = date("Ymd", strtotime("+3 day"));
		}else
		if(strcmp($dates,'day5_schedule')==0){
			$date = date("Ymd", strtotime("+4 day"));
		}else
		if(strcmp($dates,'day6_schedule')==0){
			$date = date("Ymd", strtotime("+5 day"));
		}else
		if(strcmp($dates,'day7_schedule')==0){
